	...
	{
	  window.open(url,'popupWindow','toolbar=no,location=0,directories=no,statusbar=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,screenX=0,screenY=0,top=0,left=0')
	}
    ...
    function countChar(val,limit) {
        ...
        if (len > limit) {
            ...
            document.getElementById("charNum").innerHTML = (limit - len)+' characters left.';
        }
    }
    function countChar2(val,limit) {
        var len = val.value.length;

        if (len > limit) {
            val.value = val.value.substring(0, limit);
        } else {
            document.getElementById("charNum2").innerHTML = (limit - len)+' characters left.';
        }
    }
